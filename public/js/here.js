if(navigator.geolocation){
    navigator.geolocation.getCurrentPosition(position => {
      ...
      let map = new H.Map(
        ...
        // var circle = new H.map.Circle({objLocalCoord}, 8000);
      ...
      function init(latitude, longitude, radius) {
          ...
          if(window.action == 'browse'){
              map.addEventListener('dragend', function (ev) {
                  let resultCoord = map.screenToGo(
                      ev.currentPoniter.viewportX,
                      ev.currentPoniter.viewportY
                  );
                  init(objLocalCoord.lat,resultCoord.lng,400000)
              },false);
              init(objLocalCoord.lat,objLocalCoord.lng,400000)
          }
    ...
    console.error("geolocation is not support");
}
